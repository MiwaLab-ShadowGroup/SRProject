...
public class CellAutomaton : AImageProcesser {
    ...
    public override void Processing(Mat srcMat, ref Mat dstMat)
    {
        ...
        this.myUpdate(ref srcMat, ref dstMat);
    }
    ...
    private void LifeGame_drawGrayscaleImage(ref Mat dst)
    {
        ...
        for (int y = 0; y < __h; y++)
        {
            for (int x = 0; x < __w; x++)
            {
                ...
                {
                    //要検討
                    img1.SetArray(y, x, 0xff);
                }
                else
                {
                    //要検討
                    img1.SetArray(y, x, 0x00);
                }
            }
        }
    ...
    private void myUpdate(ref Mat src, ref Mat dst)
    {
        try
        {
