...
namespace Miwalab.ShadowGroup.ImageProcesser
{
    class Canny : AShadowImageProcesser
    {
        ...
        private void Update(ref Mat src, ref Mat dst)
        {
                    ...
                    for (int j = 0; j < this.sharpness; j ++)
                        {
                            ...
                            CvPoints.Add(contour[i][j * contour[i].Length / this.sharpness + 1]);
                    }
            ...
            {
                //if(this.List_ColParticle[i] == null)
                ...
                if(i < this.List_ColParticle.Count)
                {
                    ...
                    for (int j = 0; j < this.List_ColParticle[i].Count; j++)
                    {
                        ...
                        if (this.List_ColParticle[i][j].center.X < 10) this.List_ColParticle[i][j].center = new Point(10, this.List_ColParticle[i][j].center.Y);
                        if (this.List_ColParticle[i][j].center.X > src.Width-10) this.List_ColParticle[i][j].center = new Point(src.Width-10, this.List_ColParticle[i][j].center.Y);
                        if (this.List_ColParticle[i][j].center.Y < 10) this.List_ColParticle[i][j].center = new Point(this.List_ColParticle[i][j].center.X, 10);
                        if (this.List_ColParticle[i][j].center.Y > src.Height-10) this.List_ColParticle[i][j].center = new Point(this.List_ColParticle[i][j].center.X, src.Height-10);
                ...
                else 
                {
                    ...
                    for (int j = 0; j < this.List_Contours[i].Count; j++)
                    {
                        ColParticle colPtcl = new ColParticle(this.List_Contours[i][j]);
        ...
        public class ColParticle
        {
            ...
            //    this.frameCount = 0;
            //}
            public ColParticle( OpenCvSharp.CPlusPlus.Point pt)
            {
                this.center = pt;
